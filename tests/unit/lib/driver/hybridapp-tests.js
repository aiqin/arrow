...
YUI.add('hybridApp-tests', function (Y, NAME) {

        var path = require('path'),
            util = require('util'),
            fs = require('fs'),
            arrowRoot  = path.join(__dirname, '../../../..'),
            driverClass = require(arrowRoot + '/lib/driver/hybridapp.js');

        driverClass.wdAppPath = arrowRoot + '/tests/unit/stub/hybridappwebdriver.js';
        ...
        function getHybridAppUrl_false(url) {
            return false;
        }

        function getHybridAppUrl_true(url) {
            return true;
        } 
        suite.add(new Y.Test.Case({
                'test start function when hybridAppTestServer is not set' : function () {
                ...
                'test start function when hybridAppTestServer is set and correct' : function () {
                var driver, config, start;
        ...
        'test createDriverJs when the testseed file does not exist with no arrow server running' : function () {
            ...
            'test createDriverJs when the testrunner file does not exist with no arrow server running' : function () {
                ...
                'test createDriverJs when there is incorrect file path in testparams with no arrow server running' : function () {
                   ...
                   'test createDriverJs when all the parameters are correct and arrow server running' : function () {
                      ...
                      'test navigate when the web view url is not correct' : function () {
                         ...
                         driver.getHybridAppUrl = getHybridAppUrl_false;
                         driver.navigate("testhybridAppurl", function (msg) {
                                    called = true;
                                    Y.Assert.areEqual(msg, "Cannot load web view: testhybridAppurl");
                                 });
                         Y.Assert.isTrue(called);
                      },

                     'test navigate when the web view url is correct' : function () {
                         var driver = new driverClass({hybridAppTestServer: "http://localhost:9000/shanghai"},{}), called = false;
                         driver.getHybridAppUrl = getHybridAppUrl_true;
                         driver.start(function(msg){ 
                                 Y.Assert.areEqual(msg, "create webdriver successfully");
                                 });
                     ...
                     'test executeTest when createDriverJs return false' : function () {
                         var driver = new driverClass({hybridAppTestServer: "http://localhost:9000/shanghai"},{});
                         var called = false;
                     ...
                     'test executeTest when test is html file and the url is false' : function () {
                          ...
                          driver.executeTest(correctConfig, testParams, function(msg){
                                  called = true;
                                  Y.Assert.areEqual(msg, "Cannot load the hybridApp web view: " + arrowRoot + '/lib/client/testHost.html');
                                  });
                     ...
                     'test executeTest when there is correct page in test parameter while webdriver return false' : function () {
                         var testParams = {
                            page: arrowRoot + '/lib/client/hybridAppTestHost.html', 
                         }
                         var driver = new driverClass({hybridAppTestServer:"http://localhost:9000/shanghai"}, testParams);
                         var called = false;
                    ...
                    'test executeTest when there is no page with coverage' : function () {
                        ...
                        coverageConfig = {
                            hybridAppTestServer : 'http://localhost:9000/shanghai',
                            coverage : true
                        };
             ...
             'test getHybridAppUrl when the url is a valid hybridApp app' : function(){
                 var driver = new driverClass({},{}), result = "";
                
                 var url =  "crt://yahoo.com/yahoo.someapp"; 
                 result = driver.getHybridAppUrl(url);
                 Y.Assert.areEqual(result, url);
             },

             'test getHybridAppUrl when the url is the default app url' : function(){
                 var driver = new driverClass({},{}), result = "";
                
                 var url = "lib/client/hybridAppTestHost.html";
             ...
             'test getHybridAppUrl when the url is a valid http url for browser test' : function(){
                 var driver = new driverClass({},{}), result = "";

                 result = driver.getHybridAppUrl("  http://testpage   ");
                 Y.Assert.areEqual(result, "http://testpage");
             },

             'test getHybridAppUrl when the url is a not acceptable url' : function(){
                 var driver = new driverClass({},{}), result = "";
                 result = driver.getHybridAppUrl("anyurl");
                 Y.Assert.isFalse(result);
             }
        }));
