...
YUI.add('hybridApp-controller-tests', function(Y, NAME) {

    var path = require('path'), arrowRoot = path.join(__dirname, '../../../..'), 
    hybridAppController = require(arrowRoot + '/lib/controller/hybridapp.js')
    suite = new Y.Test.Suite(NAME), A = Y.Assert, driverClass = require(arrowRoot + '/lib/driver/hybridapp.js'),self = this;

    driverClass.wdAppPath = arrowRoot + '/tests/unit/stub/hybridappwebdriver.js';

    function validateLocator(sessionId, params) {
        var wasCalled = false,config, hybridApp; 
        ...
        hybridApp = new hybridAppController({}, params, driver);
        hybridApp.execute(function(msg) {
            if (msg === null) {
