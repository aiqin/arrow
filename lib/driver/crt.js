...
//    console.log("===========in Crt Driver=============\n");
 //   console.log("the config is " + util.inspect(config) + "the args is " + util.inspect(args));
...
CrtDriver.prototype.start = function (callback) {
    var self = this,
       ...
       //TBD: here I need distinguish seleniumHost from shanghai Host
    ...
    self.webdriver.buildWebDriver(callback,this.sessionId);
};
...
CrtDriver.prototype.createDriverJs = function (testParams, callback) {
    ...
    //TBD: what is action, shall we need it?
    ...
    //TBD: these two lines to replace the arrow testrunner with crt's 
    runner = runner.substring(0, runner.length -3 ) + "1.js";
    self.logger.info("===runner:" + runner);
...
CrtDriver.prototype.navigate = function (page, callback) {
    ...
    webdriver.navigate(url, function (){
            callback();
     });
    } 
};
