...
SessionFactory.prototype.runAllTestSessions = function () {
    ...
    function startTest() {
        ...
        self.logger.info("driver="+self.args.driver);
if(self.args.driver!=="hybridApp"){//change for hybridApp driver: keep current process for selenium
        ...
        // for hybridApp driver, get connected sessions
         var baseurl = self.config.hybridAppTestServer;
         var debug = false;
        
         if (self.args.logLevel) {
             if ("debug" === self.args.logLevel.toLowerCase().trim()){
                 debug = true;
             }   
         }   

        self.logger.debug("Get shanghai sessions");
        controller = new shanghaiController({baseUrl: baseurl, nodebug:!debug});
        controller.connect(); //the connect failure is handled in shanghai server internally since we cannot listen the error message here
        var hybridAppSessions;

            controller.on('local.ready', function() {
                controller.getSessions(function(sessions) {
                    if (sessions.length === 0) {
                        controller.close();
                        self.logger.error("No hybridApp sessions found, cannot run tests!");
                    } else {
                        hybridAppSessions = sessions;
                        self.logger.debug("the sessions are:" + util.inspect(hybridAppSessions));
                        testQueues[0] = {
                            curIndex : 0,
                            sessions : []
                        };
                        for ( i = 0; i < hybridAppSessions.length; i += 1) {
                            for ( j = 0; j < tests.length; j += 1) {
                                test = clone(tests[j]);
                                testQueues[0].sessions.push(new TestSession(self.config, test, hybridAppSessions[i].sid));
                            }
                            totalSessionCount += tests.length;
                        }
                        self.logger.debug("totalSessionCount=" + totalSessionCount);
                        runNextTest(testQueues[0]);
                    }
                });
            });
            }
    ...
    if (this.reuseSession&&self.args.driver!=="hybridApp") { //change for hybridApp driver: does not support reuse session.
...
SessionFactory.prototype.tearDown = function (testQueue, wdSessions) {
    ...
    if (this.config.coverage) {
        if(this.descriptor!==undefined){
            this.logger.debug("descriptor is ["+this.descriptor+"]");
            covFile = path.resolve(this.descriptor.split(".")[0] + "-coverage.json");
        }else{
            if(this.test!==undefined){
                this.logger.debug("test is ["+this.test+"]");
                covFile = path.resolve(this.test.split(".")[0] + "-coverage.json");
            }else{
                this.logger.debug("all the tests is ["+this.tests+"]");
                covFile = path.resolve("coverage.json");
            }
        }
        this.logger.debug("temp coverage file is"+covFile);
