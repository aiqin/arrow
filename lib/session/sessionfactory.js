...
if(self.args.driver!=="crt"){//change for crt driver: keep current process for selenium
        ...
        // for CRT driver, get connected sessions
         ...
         var debug = self.args.logLevel ? true : false; 
        self.logger.debug("Get shanghai sessions");
        var controller = new shanghaiController({baseUrl: baseurl, nodebug:!debug});
        controller.connect();
        var crtSessions;
            ...
            controller.on('local.ready', function() {
                controller.getSessions(function(sessions) {
                    if (sessions.length === 0) {
                        controller.close();
                        self.logger.debug("No shanghai sessions found, cannot run tests!");
                    } else {
