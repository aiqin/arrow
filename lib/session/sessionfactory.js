...
if(self.args.driver!=="crt"){//change for crt driver: keep current process for selenium
        ...
        // for CRT driver, get connected sessions
         var baseurl = self.config.crtTestServer;
         var debug = false;
        
         if ((self.args.logLevel === "debug") || (self.args.logLevel === "DEBUG")) {
             debug = true;
         }
            ...
            controller.on('local.ready', function() {
                controller.getSessions(function(sessions) {
                    if (sessions.length === 0) {
                        ...
                        for ( i = 0; i < crtSessions.length; i += 1) {
                            for ( j = 0; j < tests.length; j += 1) {
                                test = clone(tests[j]);
                                testQueues[0].sessions.push(new TestSession(self.config, test, crtSessions[i].sid));
                            }
                            totalSessionCount += tests.length;
                        }
    ...
    if (this.reuseSession&&self.args.driver!=="crt") { //change for crt driver: does not support reuse session.
