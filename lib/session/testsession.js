...
TestSession.prototype.setup = function (callback) {
    ...
    } else if ("crt" === this.driverName) { 
        this.logger.info("Using crt driver");
        DriverClass = require("../driver/crt");
    } else {
        this.logger.fatal("ERROR :" + this.driverName + " is not a supported test driver, Please provide \"selenium\" or \"nodejs\" or \"crt\" as driver.");
        callback("ERROR :" + this.driverName + " is not a supported test driver, Please provide \"selenium\" or \"nodejs\" or \"crt\" as driver.");
        return;
    }
    ...
    if (!this.testParams.page) {
        if ("crt" === this.driverName) {
            this.testParams.page = this.config["defaultCrtTestHost"];
        } else {
    ...
    this.logger.error("====the testParams are " + util.inspect(this.testParams));
    callback(null);
};
